...
from qt.utils.linear_system import get_circ_unitary
...
def search_evolution_space_by_hdi_bitstr(qc: QuantumCircuit, params, Hd_bitstr_list, anc_idx, mcx_mode, num_qubits, shots, provider:Provider, already_set:set = set()):
    num_basis_list = []
    set_basis_list = []
    depth_list = []
    set_basis = already_set.copy()
    ...
    for i,hdi_vct in enumerate(Hd_bitstr_list):
        ...
        set_basis.update(set(counts.keys()))
        num_basis_list.append(len(set_basis))
